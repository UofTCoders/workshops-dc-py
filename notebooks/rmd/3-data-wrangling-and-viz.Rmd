...
Before continuing the exploration of the world populatino data, let's discuss how to customize the appearance of our plots. The returned object is an matplotlib axes, so all configuration available through `matplotlib` can be applied to the returned object by first assigning it to a variable name (`ax` by convention).
...
> 2. Plot the total CO2 for the world and for each region.
> 3. Create a facetted plot comparing total CO2 levels across income groups and regions.
